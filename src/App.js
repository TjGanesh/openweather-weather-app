...
function App() {
  ...
  const fetchWeather = async (e) => {
    ...
    const response = await Axios.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&cnt=5&appid=fe4feefa8543e06d4f3c66d92c61b69c`,
    );
  ...
  return (
    <>
      {city && weather ? (
          Object.keys(weather.list).map((weatherBydate, idx) => (
        <Container>
          <AppLabel>{weather.list[weatherBydate].dt_txt}</AppLabel>
          <WeatherComponent key={idx} weather={weather.list[weatherBydate]} city={weather.city} />
        </Container>
          ))
      ) : (
        <Container>
